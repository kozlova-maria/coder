...
typedef struct Node {                       //структура - узел
    unsigned char symbol;                   // беззнаковое инт от 0 до 255, 1 байт, имеют только листья
    uint32_t freq;                          // беззнаковое 32 байтовое инт
    struct Node* left;                      //указатель на левого родителя
    struct Node* right;                     //указатель на правого родителя
} Node;

// --- Подсчёт частот ---
void count_frequencies(const char* filename, uint32_t freq[256]) {
    for (int i = 0; i < 256; i++) freq[i] = 0;                      //обнуляем массив частот
    ...
    int c;
    while ((c = fgetc(file)) != EOF) {                              //каждому элементу массива с индексом - код данного символа - прибавляем 1, если встретили
        freq[c]++;
    }
    fclose(file);
}
...
// --- Создание узла ---
Node* create_node(unsigned char symbol, uint32_t freq) {            //принимает символ и частоту, возвращает узел
    Node* node = (Node*)malloc(sizeof(Node));                   //выделение памяти на структуру
    node->symbol = symbol;
    node->freq = freq;
...
void sort_nodes(Node** nodes, int n) {                              //сортировка пузырьком, медленно, неприятно, но визуально понятна и гарантирует однозначность сортировки
    for (int i = 0; i < n - 1; i++) {
...
Node* build_huffman_tree(Node** nodes, int* node_count) {           //делаем из нескольких узлов один. функция принимает массив узлов и их количество, возвращает самого дальнего предка
    int n = *node_count;
    if (n == 0) return NULL;
    if (n == 1) {
...
void generate_codes(Node* node, char* buffer, int depth, char** codes) {        //рекурсивно генерируем коды
    if (!node) return;
...
char** build_huffman_dictionary(uint32_t freq[256]) {
    char** codes = (char**)calloc(256, sizeof(char*));          //ради приличия создаем целый словарь перевода
    int unique = 0;
...
// --- Печать словаря ---
void print_dictionary(char** codes, uint32_t freq[256]) {       //ненужная функция, но можно добавить потом
...
void encode_file(const char* input_filename, const char* output_filename, uint32_t freq[256]) {     //кодирование файла
    ...
    while ((c = fgetc(in)) != EOF) {
        char* code = codes[c];
        for (int i = 0; code[i]; i++) {             //пока код не закончится, заносим биты
            if (code[i] == '1')
                byte |= (1 << (7 - bit_count));     //с конца вперед двигаем биты, пока не получим полный байт, так как сдвигаем все время, в конце останутся незначащие нули
            bit_count++;
            total_bits++;
            if (bit_count == 8) {
                fputc(byte, out);
...
void decode_file(const char* encoded_filename, const char* output_filename) {
    ...
    // 2. Посчитать, сколько уникальных символов
    int unique = 0;
    for (int i = 0; i < 256; i++) if (freq[i]) unique++;

    if (unique == 0) {
        // Пустой файл — создаём пустой output
        FILE* out = fopen(output_filename, "wb");
        if (out) fclose(out);
        printf("\nDecoding completed!\n");
        printf("Encoded file: %s\n", encoded_filename);
        printf("Output file: %s (empty)\n", output_filename);
        return;
    }

    // 3. ОСОБЫЙ СЛУЧАЙ: один символ — не читаем биты!
    if (unique == 1) {
        int sym = -1;
        uint32_t count = 0;
        for (int i = 0; i < 256; i++) {
    ...
    // 6. Пропускаем заголовок (частоты)
    uint32_t symbol_count;
    ...
    // 7. Декодируем биты, но ОСТАНАВЛИВАЕМСЯ по числу символов
    uint64_t total_symbols = 0;
    for (int i = 0; i < 256; i++) total_symbols += freq[i];
